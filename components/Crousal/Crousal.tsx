...
import ImageCrousal from '../Image/Image';
...
export default function Crousal() : JSX.Element{
    ...
    const handelPress = (event) =>{
        ...
        fraction<0.5? setCurrentIndex(Math.floor((index-1+imageURL.length) % (imageURL.length))): setCurrentIndex(Math.floor((index+1) % (imageURL.length)))
    }
